...
for i in range(1):
    ...
    file_in = open("coll_test_wo_coll_" + "matt" + ".out", "r")
    ...
    file_in = open("coll_test_w_coll_" + "matt" + ".out", "r")
    lines = file_in.readlines()
    for line in lines:
        if line and "NAN" not in line and "INF" not in line:
            x.append(float(line.split()[0]))
    ...
    if i != 1:
        ...
        axs1[0].set_ylim([-0.1, 15.])
        axs1[1].set_ylim([-0.1, 15.])
    else:
    ...
    fig1.tight_layout()
    # plt.show()
    fig.savefig(str(i + 1) + '_1_matt.png', dpi=300)
    fig1.savefig(str(i + 1) + '_2_matt.png', dpi=300)
