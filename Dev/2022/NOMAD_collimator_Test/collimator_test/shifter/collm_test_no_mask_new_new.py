...
# Sample - diamond
sfilename_noc = '/SNS/NOM/IPTS-28922/nexus/NOM_168865.nxs.h5'
...
# MT Shifter
mtfilename_noc = "/SNS/NOM/IPTS-28922/nexus/NOM_168013.nxs.h5"
mtiname_noc = 'MT_wo_coll'

# Cal
calfile_noc = '/SNS/NOM/shared/CALIBRATION/Collimator_Test/NOMAD_168865_2022-01-26_shifter.h5'
...
# MT Shifter
mtfilename_c = "/SNS/NOM/IPTS-30147/nexus/NOM_183704.nxs.h5"
mtiname_c = 'MT_w_coll'
...
for ct, mask_det in enumerate(mask_det_ids):
    ...
    file_out = open('coll_test_wo_coll_van_' + str(ct + 1) + ".out", "w")
    for count, item in enumerate(x_val):
    ...
    file_out = open('coll_test_w_coll_van_' + str(ct + 1) + ".out", "w")
    for count, item in enumerate(x_val):
        file_out.write("{0:20.5F}{1:20.5F}\n".format(item, van_out[count]))
    file_out.close()
